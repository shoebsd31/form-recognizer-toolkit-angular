...
export const makeLayerFilter = (layerName: string) => {
    return {
        layerFilter: (layer: Layer) => layer.get(LAYER_NAME) === layerName,
    };
};

export const makeVectorLayer = (layerName: string, options = {}): Layer => {
...
export const makeImageLayer = (layerName: string, uri: string, projection: Projection, imageExtent: Extent, options = {}): Layer => {
...
export const getBoundingBoxFromFeatureId = (id: string) => id.split(",").map(parseFloat);
